...
## V0.38.1 — all-in conservador no replay

A suíte possui 603 testes. As regressões de `tests/realHandDecisionReplay.test.ts` distinguem mesa 2-max explícita, na qual bet/raise all-in imediatamente anterior remove Raise, de mesa 6-max e `maxPlayers` desconhecido, nas quais Raise é preservado. As cinco famílias, inclusão histórica, anti-hindsight e isolamento de storage/evidência continuam cobertos. Executar `npm test`, `npm run typecheck`, `npm run build` e `git diff --check`.
