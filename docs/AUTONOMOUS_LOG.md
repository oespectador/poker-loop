...
## 2026-08-24 — V0.38.1 All-in conservador no replay

A regra anterior removia Raise diante de qualquer bet/raise all-in imediatamente anterior. Isso inferia ilegalidade sem informação suficiente em pots multiway. A correção agora exige `maxPlayers === 2`; em mesas maiores ou com lotação desconhecida, Raise permanece como possibilidade conservadora. Não foram adicionados stacks, side pots, reopening rules ou motor de ações legais. Anchor, anti-hindsight, comparação, estado efêmero e isolamento da V0.38 permanecem inalterados. **Validação:** 603 testes, typecheck, build e `git diff --check`.
...
## 2026-08-24 — V0.37 Padrão observado → sugestão de próximo treino

**Hipótese de trabalho:** depois das cinco novas decisões e do encerramento válido, destacar caminhos editoriais relacionados reduz a carga da escolha sem transformar autorrelato em diagnóstico.
