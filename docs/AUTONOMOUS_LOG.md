...
Adicionar validação automática para pushes na `main` e pull requests direcionados à `main`. O workflow faz checkout, configura Node 20, instala dependências e executa testes, typecheck e build.

### Estratégia de instalação

Como o repositório não possui `package-lock.json`, o workflow usa `npm install`: `npm ci` exige um lockfile existente. Nenhuma dependência, deploy, secret ou cache foi adicionado.

### Comportamento antes/depois

O código e o comportamento do produto não mudaram. A única mudança executável é a validação de CI; scheduler, exercícios, UI, navegação e storage permanecem intactos.

### Validação e limitações

- `npm test`, `npm run typecheck`, `npm run build` e `git diff --check`: aprovados localmente;
- sintaxe YAML validada localmente com o parser Psych do Ruby;
- o workflow não pode ser executado integralmente fora do ambiente do GitHub Actions; a primeira execução remota confirmará a integração com os actions hospedados;
