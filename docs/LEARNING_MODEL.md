...
## V0.26 — comparação descritiva entre janelas congeladas

A sequência factual passa a ser `descrição retrospectiva (V0.18) → hipótese (V0.19) → primeira observação prospectiva (V0.20) → memória histórica (V0.21) → ponte voluntária (V0.22) → sessão iniciada (V0.23) → sessão concluída (V0.24) → segunda observação prospectiva (V0.25) → comparação descritiva (V0.26)`. Quando a proveniência por identidade e a sequência temporal `janela original → episódio → sessão → follow-up` são exatas, a janela original e cada follow-up concluído do mesmo `ReasoningFactor` podem ser apresentados lado a lado. A conclusão de cada janela é o timestamp factual de sua quinta observação; `episode.endedAt` data o arquivamento, não o fim da janela original. A Skill registra apenas o foco escolhido para a sessão entre elas. Dois conjuntos autorrelatados de cinco decisões não constituem, por si sós, evidência de eficácia do treinamento.
