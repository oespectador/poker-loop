...
function raw(id: string, options: { river?: boolean; riverAggression?: boolean; shows?: boolean; allIn?: boolean; turnPressure?: boolean; contribution?: number; winner?: "Hero" | "Villain" } = {}) {
  const river = options.river || options.riverAggression;
  return [`Poker Hand #${id}: Hold'em No Limit ($1/$2) - 2026/08/23 12:00:00`, "Table 'Alpha' 6-max Seat #1 is the button", "Seat 1: Hero ($100 in chips)", "Seat 2: Villain ($100 in chips)", "Dealt to Hero [Ah Kh]", "Hero: calls $2", "*** FLOP *** [As 7d 2c]", "Villain: bets $5", "Hero: calls $5", "*** TURN *** [As 7d 2c] [9h]", ...(options.turnPressure ? ["Villain: bets $8"] : []), "Hero: checks", ...(river ? ["*** RIVER *** [As 7d 2c 9h] [3s]", ...(options.riverAggression ? ["Villain: bets $10"] : []), `Hero: ${options.allIn ? "raises $20 to $90 and is all-in" : `calls $${options.contribution ?? 10}`}`] : []), "*** SHOW DOWN ***", ...(options.shows ? ["Hero: shows [Ah Kh]"] : []), `${options.winner ?? "Hero"} collected $200 from pot`].join("\n");
}
function candidate(id: string, options = {}, reason: HandReviewSuggestionReason = "high-commitment"): HandReviewSuggestion { return { id: `${reason}:${id}`, source: "gg-pokercraft", sourceHandId: id, reason, createdAt: now, heroCards: ["Ah", "Kh"], playedAt: now, reasonLabel: "ESTRUTURA", reasonMessage: "Característica observável.", rawHandText: raw(id, options) }; }
function batch(candidates: HandReviewSuggestion[], surfacedSuggestionIds: string[] = []): ActiveGgImportBatch { return { version: 1, fingerprint: "a".repeat(64), importedAt: now, recognizedHands: candidates.length, ignoredHands: 0, candidates, surfacedSuggestionIds }; }
...
test("seleção filtrada mantém a ordem original do pool", () => { const items = [candidate("1"), candidate("2", { river: true }), candidate("3"), candidate("4", { river: true })]; assert.deepEqual(nextImportCandidatesForFilter(batch(items), "river-decision", 5, []).map(({ sourceHandId }) => sourceHandId), ["2", "4"]); });
test("seleção filtrada ignora surfaced e sourceHandId já pending", () => { const items = [candidate("1", { river: true }), candidate("2", { river: true }), candidate("3", { river: true })]; assert.deepEqual(nextImportCandidatesForFilter(batch(items, [items[0].id]), "river-decision", 5, [{ ...items[1], id: "pending:other" }]).map(({ sourceHandId }) => sourceHandId), ["3"]); });
test("seleção filtrada respeita pedido, filtro vazio e teto de quinze", () => { const items = Array.from({ length: 20 }, (_, index) => candidate(String(index), { river: true })); assert.equal(nextImportCandidatesForFilter(batch(items), "river-decision", 5, []).length, 5); assert.deepEqual(nextImportCandidatesForFilter(batch(items), "hero-showdown", 5, []), []); assert.equal(nextImportCandidatesForFilter(batch(items), "river-decision", 5, Array.from({ length: 13 }, (_, i) => ({ ...candidate(`p${i}`), sourceHandId: `p${i}` }))).length, 2); assert.equal(MAX_PENDING_HAND_SUGGESTIONS, 15); });
test("consumo pelo filtro também remove a mão do +5 geral e vice-versa", () => { const items = [candidate("1", { river: true }), candidate("2", { river: true })]; const filtered = surfaceImportCandidatesForFilter(batch(items), "river-decision", 1, []); assert.deepEqual(nextImportCandidates(filtered.batch, 5, []).map(({ sourceHandId }) => sourceHandId), ["2"]); const generallySurfaced = { ...batch(items), surfacedSuggestionIds: [items[0].id] }; assert.deepEqual(nextImportCandidatesForFilter(generallySurfaced, "river-decision", 5, []).map(({ sourceHandId }) => sourceHandId), ["2"]); });
test("helper filtrado não altera batch nem candidates diretamente", () => { const item = candidate("1", { river: true }); const current = batch([item]); const before = structuredClone(current); surfaceImportCandidatesForFilter(current, "river-decision", 5, []); assert.deepEqual(current, before); assert.deepEqual(item, before.candidates[0]); });
test("transação filtrada faz rollback no segundo write e mantém disponibilidade", () => { const item = candidate("1", { river: true }); const previous = batch([item]); const next = surfaceImportCandidatesForFilter(previous, "river-decision", 5, []); const previousRaw = JSON.stringify(previous); const values = new Map([[ACTIVE_GG_IMPORT_BATCH_KEY, previousRaw], [HAND_SUGGESTIONS_KEY, "[]"]]); failingStorage(values); assert.throws(() => persistGgImportTransition(previous, [], next.batch, next.suggestions)); assert.equal(values.get(ACTIVE_GG_IMPORT_BATCH_KEY), previousRaw); assert.equal(countRemainingCandidatesByFilter(JSON.parse(values.get(ACTIVE_GG_IMPORT_BATCH_KEY)!))["river-decision"], 1); cleanup(); });
