...
export function isPostTrainingRealHandFollowUp(value: unknown): value is PostTrainingRealHandFollowUp {
  ...
  if (item.version !== 1 || ![item.id, item.episodeId, item.sessionId].every((id) => typeof id === "string" && Boolean(id.trim())) || !factors.has(item.factor as ReasoningFactor) || !timestamp(item.startedAt) || !strings(item.baselineHandReviewIds) || !Array.isArray(item.observations) || item.observations.length > 5) return false;
  ...
  return valid && (item.windowCompletedAt === undefined ? item.observations.length < 5 : item.observations.length === 5 && timestamp(item.windowCompletedAt) && item.windowCompletedAt === (item.observations.at(-1) as ProspectiveObservedReview).createdAt);
}
...
const chronological = (a: StoredRealHandReasoningSnapshot, b: StoredRealHandReasoningSnapshot) => Date.parse(a.createdAt) - Date.parse(b.createdAt) || a.id.localeCompare(b.id);
