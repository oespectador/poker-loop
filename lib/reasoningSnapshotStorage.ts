import type { ParsedActionType, RealHandReasoningSnapshot, ReasoningFactor, SelfRatedSupport, StoredRealHandReasoningSnapshot } from "./types";
...
export function isRealHandReasoningSnapshot(value: unknown): value is StoredRealHandReasoningSnapshot {
  ...
  if (item.sourceHandId !== undefined && (typeof item.sourceHandId !== "string" || !item.sourceHandId.trim())) return false;
  if (!streets.has(source.street as string) || !Number.isInteger(source.sequenceIndex) || (source.sequenceIndex as number) < 0 || !actions.has(source.action as ParsedActionType) || !validMoney(source.amount) || !validMoney(source.toAmount) || !validOptionalBoolean(source.allIn)) return false;
...
export function readReasoningSnapshots(): StoredRealHandReasoningSnapshot[] { if (typeof window === "undefined") return []; try { const parsed: unknown = JSON.parse(window.localStorage.getItem(REASONING_SNAPSHOTS_KEY) ?? "[]"); if (!Array.isArray(parsed)) return []; const valid = parsed.filter(isRealHandReasoningSnapshot); return [...new Map(valid.map((x) => [x.handReviewId, x])).values()].sort((a,b) => a.handReviewId.localeCompare(b.handReviewId)); } catch { return []; } }
function write(items: StoredRealHandReasoningSnapshot[]) { if (typeof window !== "undefined") window.localStorage.setItem(REASONING_SNAPSHOTS_KEY, JSON.stringify(items)); }
export function saveReasoningSnapshot(snapshot: RealHandReasoningSnapshot): RealHandReasoningSnapshot { if (!isRealHandReasoningSnapshot(snapshot) || !snapshot.sourceHandId) throw new Error("Snapshot inválido."); const current = readReasoningSnapshots().find((x) => x.handReviewId === snapshot.handReviewId); const saved = current ? { ...snapshot, id: current.id, createdAt: current.createdAt } : snapshot; write([...readReasoningSnapshots().filter((x) => x.handReviewId !== saved.handReviewId), saved]); return saved; }
