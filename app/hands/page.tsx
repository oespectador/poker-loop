...
export default function HandsPage() {
  ...
  function promote(item: HandReviewSuggestion) {
    try {
      const title = `${item.heroCards.map(cardLabel).join(" ")} · ${dateLabel(item.playedAt)}`;
      const saved = saveRealHand(createRealHand({ ...emptyForm, title, rawHandText: item.rawHandText })); removeHandSuggestion(item.id);
      setHands(readRealHands()); setSuggestions(readHandSuggestions()); setSelectedSuggestionId(undefined); setSelectedId(saved.id); setMessage("Mão salva para revisão. Complete sua reflexão quando quiser.");
    } catch { setImportMessage("Não foi possível salvar esta mão no armazenamento deste navegador."); }
  }
  function submit(event: FormEvent) { event.preventDefault(); const validation = validateRealHandInput(form); if (validation) return setMessage(validation); try { const saved = editingId ? updateRealHand(editingId, form) : saveRealHand(createRealHand(form)); if (!saved) return setMessage("Esta mão não está mais disponível."); setHands(readRealHands()); setSelectedId(saved.id); setEditingId(undefined); setForm(emptyForm); setMessage("Mão salva."); } catch { setMessage("Não foi possível salvar a mão no armazenamento deste navegador. Seus dados existentes não foram apagados."); } }
  function edit(hand: RealHandReview) { const { id: _id, createdAt: _createdAt, ...input } = hand; setEditingId(hand.id); setForm(input); setSelectedId(undefined); setMessage(""); }
  function remove(hand: RealHandReview) { if (!window.confirm("Excluir esta mão registrada?")) return; deleteRealHand(hand.id); setHands(readRealHands()); setSelectedId(undefined); setEditingId(undefined); setMessage("Mão excluída. Seu progresso de treino não foi alterado."); }
  return <AppShell>
